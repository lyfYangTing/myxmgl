 ...
 $("#cha").click(function(){
	...
	$("#link1").click(function(){
		...
		if(p<1){
		 ...
		 $("#fom").attr("action","<%=basePath%>/messageServlet.do?op=showAllUserInform&caozuo=fjr&page="+p);
	});
     ...
     $("#link").click(function(){
        $("#fom").attr("action","<%=basePath%>/userServlet.do?op=getAllUsers&caozuo=fjr");
     });
     ...
     $("#linkdel").click(function(){
     	var size = $(":checkbox").length;
			...
			for (var i=0;i<size;i++){	
	    	 ...
	    	 $("#fom").attr("action","<%=basePath %>/messageServlet.do?op=deleteSomeInform&caozuo=fjr");
     });
...
function linkdel(){
	...
	//删除所有选中的信息
	document.getElementById("fom").action="<%= basePath%>/infoServlet.do?op=deleteAllInfo";
              ...
              <tr>
                <td height="40" class="font42"><table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#464646" class="newfont03">
                  ...
                  <tr>
                   ...
                   <c:forEach items="${requestScope.util.list }" var="info">
                   		<tr>
                   			<td bgcolor="#FFFFFF"><input type="checkbox" name="delid"/></td>
                   			<td height="20" bgcolor="#FFFFFF">${info.ititle}</td>
                   			<td bgcolor="#FFFFFF">${info.isjr}</td>
                   			<td bgcolor="#FFFFFF">${info.iftime}</td>
                    		<td bgcolor="#FFFFFF">
                            <a href="<%=basePath %>/messageServlet.do?op=deleteOneInform&caozuo=fjr&id=${info.iid }">删除|</a><a href="<%=basePath %>/messageServlet.do?op=showOneInform&caozuo=fjr&id=${info.iid }">查看</a>
                            </td>
                    		<input type='hidden' name='hello' id="hello" value='${info.iid}'> 
        ...
        <tr>
          <td height="33"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="right-font08">
              <tr>
                ...
                [<a href="<%=basePath %>/messageServlet.do?op=showAllUserInform&caozuo=fjr&page=1" class="right-font08">首页</a> | 
                 <a href="<%=basePath %>/messageServlet.do?op=showAllUserInform&caozuo=fjr&page=${requestScope.curpage-1>0?requestScope.curpage-1 :1}" class="right-font08">上一页</a> |
                 <a href="<%=basePath %>/messageServlet.do?op=showAllUserInform&caozuo=fjr&page=${requestScope.curpage<requestScope.util.totalPage?requestScope.curpage+1:requestScope.util.totalPage}" class="right-font08">下一页</a> | 
                 <a href="<%=basePath %>/messageServlet.do?op=showAllUserInform&caozuo=fjr&page=${requestScope.util.totalPage}" class="right-font08">末页</a>]转至：
                </td>
